...
def blackjack(play_game):
  ...
  if play_game == "y":
    ...
    repeat = True
    while repeat:
      if computer_score < 21 and players_score < 21:
        ...
        if another_card == "y":
          ...
          players_card = random.choice(cards)
          players_cards.append(players_card)
          players_score = players_score + players_cards[index]
          print(f"Your cards: {players_cards}, current score: {players_score}")
          computer_card = random.choice(cards)
          computer_cards.append(computer_card)
          computer_score = computer_score + computer_cards[index]
          print(f"Computer's first card: {computer_cards[0]}")
        elif another_card == "n":
          if computer_score < 21 and players_score < 21:
